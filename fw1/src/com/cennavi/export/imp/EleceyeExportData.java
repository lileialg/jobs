...
public class EleceyeExportData extends ExportDataAbs implements ExportData {

	@Override
	public byte[] export(String layerName, int x, int y, int z) throws Exception {

		String sql = "select pid,kind,linkpid,direct,angle,speed_limit,st_astext(geom) wkt from electronic_eye a where st_intersects(a.geom,st_geometryfromtext(?,4326)) = true";
		...
		while (resultSet.next()) {
			...
			attr.put("linkpid", resultSet.getInt("linkpid"));
			
			attr.put("direct", resultSet.getInt("direct"));
			
			attr.put("speed_limit", resultSet.getInt("speed_limit"));
			
			attr.put("angle", resultSet.getDouble("angle"));
