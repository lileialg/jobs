...
public class PointaddrExportData extends ExportDataAbs implements ExportData {

	@Override
	public byte[] export(String layerName, int x, int y, int z) throws Exception {

		String sql = "select pid,locate_link_pid,locate_link_side,amdin_id,fullname_zh,fullname_en,roadadress_zh,roadadress_en,st_astext(geom) wkt from point_address a where st_intersects(a.geom,st_geometryfromtext(?,4326)) = true";
		...
		while (resultSet.next()) {
			...
			attr.put("locate_link_pid", resultSet.getInt("locate_link_pid"));
			
			attr.put("locate_link_side", resultSet.getInt("locate_link_side"));
			
			attr.put("admin_id", resultSet.getInt("admin_id"));
			
			attr.put("fullname_zh", resultSet.getString("fullname_zh"));
			
			attr.put("fullname_en", resultSet.getString("fullname_en"));
			
			attr.put("roadadress_zh", resultSet.getString("roadadress_zh"));
			
			attr.put("roadadress_en", resultSet.getString("roadadress_en"));
