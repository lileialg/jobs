...
public class BuildingExportData extends ExportDataAbs implements ExportData {

	@Override
	public byte[] export(String layerName, int x, int y, int z) throws Exception {

		String sql = "select pid,kind,massing,height,wall_meterial,name_zh,name_en,st_astext(geom) wkt from building a where st_intersects(a.geom,st_geometryfromtext(?,4326)) = true";
		...
		while (resultSet.next()) {
			...
			attr.put("massing", resultSet.getBoolean("form"));
			
			attr.put("height", resultSet.getDouble("height"));
			
			attr.put("wall_meterial", resultSet.getInt("wall_meterial"));
