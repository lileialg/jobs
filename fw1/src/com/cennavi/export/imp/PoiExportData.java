...
public class PoiExportData extends ExportDataAbs implements ExportData {

	@Override
	public byte[] export(String layerName, int x, int y, int z) throws Exception {

		String sql = "select pid,class,subclass,kindcode,level,admin,admin_brief,name_zh,name_en,st_astext(geom) wkt from poi a where st_intersects(a.geom,st_geometryfromtext(?,4326)) = true";
		...
		while (resultSet.next()) {
			...
			attr.put("admin", resultSet.getInt("admin_id"));
			
			attr.put("class", resultSet.getString("class"));
			
			attr.put("subclass", resultSet.getString("subclass"));
			
			attr.put("kindcode", resultSet.getString("kindcode"));
			
			attr.put("level", resultSet.getString("level"));
			
			attr.put("name_brief", resultSet.getString("name_brief"));
