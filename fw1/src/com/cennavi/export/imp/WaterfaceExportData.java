...
public class WaterfaceExportData extends ExportDataAbs implements ExportData {

	@Override
	public byte[] export(String layerName, int x, int y, int z) throws Exception {

		String sql = "select pid,kind,form,display_class,scale,name_zh,name_en,st_astext(geom) wkt from water_face a where st_intersects(a.geom,st_geometryfromtext(?,4326)) = true";
		...
		while (resultSet.next()) {
			...
			attr.put("display_class", resultSet.getInt("display_class"));
			
			attr.put("scale", resultSet.getInt("scale"));
