...
public class GdbKafka2Redis extends Thread {
	...
	public GdbKafka2Redis(String dir) throws FileNotFoundException,
			IOException {
	...
	public void run() {
		...
		while (iterator.hasNext()) {
			...
			byte[] value = new byte[bytes.length - 38];
			
			for(int i=0;i<38;i++){
				keys[i] = bytes[i];
			}
			
			for(int i=38;i<bytes.length;i++){
				value[i-38] = bytes[i];
			}
			
			pipeline.set(keys, value);
